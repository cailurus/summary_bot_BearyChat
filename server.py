...
def coupet():
    ...
    inputs = raw_inputs["content"]
    if type(inputs) == list:
        inputs[0] = " ".join(word_tokenize(inputs[0])).lower()
    else:
        inputs = [" ".join(word_tokenize(inputs)).lower()]

    if len(inputs[0].split()) < 20:
        return jsonify({"title": "too short to generate"})
    ...
    for i in np.argsort(indices):
        ...
        for hypo, pos_scores, align in zip(result.hypos, result.pos_scores, result.alignments):
            print(hypo, pos_scores)
            return jsonify({"title": hypo.split("\t")[-1]})
